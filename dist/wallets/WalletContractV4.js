...
class WalletContractV4 {
    ...
    async getPluginList(provider) {
        const state = await provider.getState();
        if (state.state.type !== 'active') {
            return [];
        }
        const res = await provider.get('get_plugin_list', []);
        const listReader = new core_1.TupleReader(res.stack.readLispList());
        const plugins = [];
        while (listReader.remaining > 0) {
            ...
            plugins.push(core_1.Address.parseRaw(`${workchain}:${addressHex}`));
        }
        return plugins;
    }
    /**
    ...
    async sendPluginAction(provider, args) {
        const action = this.createPluginAction(args);
        await this.send(provider, action);
    }
    createPluginAction(args) {
        return (0, createWalletTransfer_1.createWalletPluginActionV4)({
            ...args,
            sendMode: args.sendMode ?? core_1.SendMode.PAY_GAS_SEPARATELY,
            walletId: this.walletId
        });
